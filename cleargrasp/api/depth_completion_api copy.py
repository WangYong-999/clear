...
class DepthToDepthCompletion(object):
    ...
    FOLDER_MAP = {
        ...
        'filtered-output-depth': {
            'postfix': '-filtered-output-depth.exr',
            'folder-name': 'filtered-output-depth'
        },
        'filtered-output-point-cloud': {
            'postfix': '-filtered-output-pointcloud.ply',
    ...
    def store_depth_completion_outputs(self, root_dir, files_prefix, min_depth=0.0, max_depth=1.5):
        ...
        filtered_output_ptcloud_dir = os.path.join(root_dir, self.FOLDER_MAP['filtered-output-point-cloud']['folder-name'])
        ...
        utils.write_point_cloud(output_ptcloud_filename, self.surface_normals_rgb, self.output_depth, self.fx, self.fy,
                                self.cx, self.cy)
        # yong
        filtered_output_depth_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['filtered-output-depth']['postfix']
        filtered_output_depth_filename = os.path.join(filtered_output_depth_dir, filtered_output_depth_filename)
        utils.exr_saver(filtered_output_depth_filename, self.filtered_output_depth, ndim=3)

        filtered_output_ptcloud_filename = ('{:09d}'.format(files_prefix) + self.FOLDER_MAP['filtered-output-point-cloud']['postfix'])
        filtered_output_ptcloud_filename = os.path.join(filtered_output_ptcloud_dir, filtered_output_ptcloud_filename)
        utils.write_point_cloud(filtered_output_ptcloud_filename, self.surface_normals_rgb, self.filtered_output_depth, self.fx, self.fy,
                                self.cx, self.cy)
        ...
        # yong
        filtered_output_depth_filename = '{:09d}'.format(files_prefix) + '-filtered-output-depth-rgb.png'
        filtered_output_depth_filename = os.path.join(filtered_output_depth_dir, filtered_output_depth_filename)
        imageio.imwrite(filtered_output_depth_filename, filtered_output_depth_rgb)
